...
Azure Functions app (.NET 8, isolated worker) that receives SQL Server data changes in an event-driven manner with the SQL Server Change Tracking/CDC feature enabled. The changed data payload is received by an Azure Function via the SqlTrigger function input binding.
...
- Durable Functions:
  - Orchestration for retry: `RetryOrchestrator`
...
- The `RetryOrchestrator` is started on the occurance of the first failed attempt. It will monitor and take over when the built-in retry of 5 times is exhausted. The orchestration will retry posting the data based on the configured retry interval and maximum number of retries.
- `NotifyOnRetryCount` can be set in the `local.settings.json` file. When the orchestration reaches this retry count number, it will start the `NotifyOrchestrator` to send a notification (e.g., email) about the persistent failure. Note: The `NotifyOnRetryCount` setting is only used in the `RetryOrchestrator`and is not checked by the built-in 5 retries of the SQL function trigger.
...
- `RetryFunctions/RetryFunctions.cs`: durable orchestration (`RetryOrchestrator`) and activity for retry logic.
...
- Avoid deploying to the Consumption Flex plan. Scaling to multiple instances has not been tested. It should not be a problem since the SQL trigger manages leases and high-water marks per instance, but this has not been verified.
