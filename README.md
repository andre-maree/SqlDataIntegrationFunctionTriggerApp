...
Only the latest data changes are delivered. This is not a change log; rather, it provides the current state of changed rows in near real time. The app processes the changes, removes unneeded columns , HTTP posts changes to an endpoint, and uses Durable Functions for retries, notifications, and entity state persisting the retry count and last error info.
...
- The fuction SQL trigger will be called 5 times (set in `host.json` -> `extensions.queues.maxDequeueCount`) if it keeps failing to post the data. Failures are saved via the `LastError` entity.
...
Notes:
- For client-driven column projection, use `AllowedColumns` entity via the `SaveClientAllowedColumns` function.
